...
class YoloLayer(nn.Module):
	def __init__(self, anchor_mask=[], num_classes=0, anchors=[], num_anchors=1, device = None):
	...
	def forward(self, output, nms_thresh):
		...
		boxes = get_region_boxes(output.data, self.thresh, self.num_classes, masked_anchors, len(self.anchor_mask), self.device)
...
class Darknet(nn.Module):
	...
	def forward(self, x, nms_thresh):            
		...
		for block in self.blocks:
			...
			if block['type'] == 'net':
				...
				boxes = self.models[ind](x.to(self.device), nms_thresh)
	...
	def create_network(self, blocks):
		models = nn.ModuleList().to(self.device)
		...
		for block in blocks:
			if block['type'] == 'net':
				...
				yolo_layer = YoloLayer(self.device).to(self.device)
				...
				out_filters.append(prev_filters)
				out_strides.append(prev_stride)
				models.append(yolo_layer.to(self.device))
			else:
				print('unknown type %s' % (block['type']))
...
def get_region_boxes(output, conf_thresh, num_classes, anchors, num_anchors, only_objectness = 1, validation = False, use_cuda=False):
	...
	all_boxes = []
	output = output.view(batch*num_anchors, 5+num_classes, h*w).transpose(0,1).contiguous().view(5+num_classes, batch*num_anchors*h*w).to(device)
